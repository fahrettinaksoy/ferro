...
pub fn validate_kdf(n: u32, r: u32, p: u32) -> FerroResult<u8> {
    if n > (1 << 20) || r > 16 || p > 4 || !n.is_power_of_two() || n < 2 {
        return Err(FerroError::new(FerroErrorCode::Validation, "Desteklenmeyen KDF parametreleri"));
    }
    Ok(n.trailing_zeros() as u8)
}

/// scrypt anahtar türetimi (32 bayt).
pub fn derive_key(password: &str, salt: &[u8], n: u32, r: u32, p: u32) -> FerroResult<[u8; KEY_LEN]> {
...
pub fn gcm_decrypt(key: &[u8; KEY_LEN], iv: &[u8], data: &[u8]) -> FerroResult<Vec<u8>> {
    if iv.len() != IV_LEN || data.len() < TAG_LEN {
        return Err(FerroError::new(FerroErrorCode::Validation, "Şifreli veri eksik/bozuk"));
    }
    let cipher = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(key));
    cipher
        .decrypt(Nonce::from_slice(iv), data)
        .map_err(|_| FerroError::new(FerroErrorCode::AuthFailed, "Parola hatalı ya da veri bozulmuş"))
}
...
pub fn unb64(s: &str) -> FerroResult<Vec<u8>> {
    B64.decode(s).map_err(|_| FerroError::new(FerroErrorCode::Validation, "Geçersiz base64"))
}
pub fn key_from_b64(s: &str) -> FerroResult<[u8; KEY_LEN]> {
    let bytes = unb64(s)?;
    if bytes.len() != KEY_LEN {
        return Err(FerroError::new(FerroErrorCode::Validation, "Anahtar 32 bayt olmalı"));
    }
