        ...
        match self {
            Provider::Gist {
                ...
                let created: serde_json::Value = resp
                    .json()
                    .await
                    .map_err(|e| http_err(e, "Gist yanıtı okunamadı"))?;
                Ok(created
                    .get("id")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()))
            ...
            Provider::Webdav {
                ...
                let resp = put(&c)
                    .send()
                    .await
                    .map_err(|e| http_err(e, "WebDAV PUT başarısız"))?;
                if resp.status().as_u16() == 409 {
                    // Koleksiyon yok — oluştur, sonra tekrar dene.
                    let mut mkcol = c.request(
                    ...
                    let retry = put(&c)
                        .send()
                        .await
                        .map_err(|e| http_err(e, "WebDAV PUT (retry) başarısız"))?;
        ...
        match self {
                    ...
                    if let Some(raw) = file.get("raw_url").and_then(|u| u.as_str()) {
                        ...
                        return Ok(Some(
                            raw_resp
                                .text()
                                .await
                                .map_err(|e| http_err(e, "İçerik okunamadı"))?,
                        ));
                    }
                }
                Ok(file
            ...
            Provider::Webdav {
                ...
                let resp = r
                    .send()
                    .await
                    .map_err(|e| http_err(e, "WebDAV GET başarısız"))?;
                if resp.status().as_u16() == 404 {
                    return Ok(None);
                }
                check_status(resp.status(), false)?;
...
fn check_status(status: reqwest::StatusCode, have_id: bool) -> FerroResult<()> {
    ...
    if code == 401 || code == 403 {
        return Err(FerroError::new(
            ...
            "Depo kimlik doğrulaması başarısız (jeton/parola)",
        ));
    }
    if code == 404 && have_id {
        return Err(FerroError::new(
            FerroErrorCode::NotFound,
            "Uzak kayıt bulunamadı",
        ));
    }
    Err(FerroError::with_detail(
        FerroErrorCode::ConnectionFailed,
        "Depo isteği başarısız",
        format!("HTTP {code}"),
    ))
}
